...
- **CD is CapRover, not Kamal** — `.github/workflows/cd.yml` deploys on every push to `main`, but only after polling `ci.yml` for the same SHA and seeing it green (the ruleset on `main` lets the owner push straight through, so for owner pushes that poll is the only gate). The image is **built on the Actions runner** (`docker/build-push-action`, from the root `Dockerfile`, layer cache in `type=gha`) and pushed to `ghcr.io/romulooliveira94/trabalhei-plus:<sha7>` plus `:latest`; `caprover/deploy-from-github` then hands CapRover just the image name, so the server only `docker pull`s — **it no longer builds anything**. `captain-definition` is still versioned but **unused by CD**: it exists for a manual tarball deploy (`caprover deploy`), where CapRover does build server-side. Kamal stays only for a separate, Kamal-managed deploy: its `bin/kamal console|shell|logs|dbc` aliases find Kamal-published containers, **not** CapRover's (`srv-captain--<app>`), so they are not the way into this production. `config/deploy.yml` is still placeholder (server `192.168.0.1`, registry `localhost:5555`) — override before using it.
- **CapRover essentials** — repo secrets `CAPROVER_SERVER`, `CAPROVER_APP_NAME`, `CAPROVER_APP_TOKEN` (App Token from the app's Deployment tab) — unchanged by the ghcr.io switch; the image push authenticates with the workflow's own `GITHUB_TOKEN` (job-level `permissions: packages: write`), so **no PAT lives in the repo**. Server-side prerequisite: ghcr.io registered under **Cluster → Docker Registries** with a classic PAT that has `read:packages` — GHCR packages are private by default, and without it the deploy dies on `docker pull ... unauthorized`. Rollback is CapRover → the app → **Deployment → Deploy via ImageName** with an older `<sha7>` tag (every deployed SHA stays in the registry). App env: `RAILS_MASTER_KEY` (mandatory), `GOOGLE_CLIENT_ID`/`GOOGLE_CLIENT_SECRET` (read from `ENV` in `config/initializers/devise.rb`, *not* credentials), `SOLID_QUEUE_IN_PUMA=true` from day one (prod runs Solid Queue inside Puma; without it any future `deliver_later`/`perform_later` is enqueued and never runs). Persistent directory `/rails/storage` is mandatory — all four SQLite files live there and every deploy wipes them without it. **Its label must be unique across the whole CapRover instance**: CapRover maps a label to the Docker volume `captain--<label>` with **no app prefix**, so every app on the instance shares that namespace. A generic label (`db`, `storage`) mounts another app's volume at `/rails/storage`, and boot-time `db:prepare` then finds a foreign, already-initialized database, replays this app's migrations onto it and crash-loops on `SQLite3::SQLException: table "users" already exists` (absent a table-name clash it would instead migrate the other app's database). The label in use is **`trabalhei-plus-db`** → `/rails/storage`. Health check `/up`, port 80 (Thruster), TLS terminated by CapRover's nginx with `force_ssl`/`assume_ssl` left commented out in `config/environments/production.rb`.
...
- Host: GitHub, `RomuloOliveira94/trabalhei-plus`. Work goes **straight to `main`** — no branch/PR flow. Commit locally; the user pushes.
- `main` has a GitHub **ruleset**: the owner (repo admin) can push directly, so the straight-to-`main` flow above still works; anyone else must open a PR that passes the 5 CI checks and gets a code-owner approval (`.github/CODEOWNERS` makes the owner the code owner of every path).
- Deployment target: **CapRover**, pulling the image CD publishes to `ghcr.io` (`.github/workflows/cd.yml` — see Gotchas). Prod access goes through CapRover (App Logs / `docker exec` on the host — see Commands). Kamal's `config/deploy.yml` is kept for a separate Kamal-managed deploy and still needs a real server + registry before it works.